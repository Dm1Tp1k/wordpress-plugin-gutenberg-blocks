...
class SafervpnPricingCardsBestVpn {

    const SCRIPT_NAME = 'safervpn-block-pricing-cards-best-vpn';
    ...
    public static function renderCallback($attributes) {
        ...
        ob_start();
        require('pricing-cards-best-vpn.tpl.php');
        return ob_get_clean();
    }
    ...
    public static function init()
    {
        wp_register_script(
            self::SCRIPT_NAME,
            plugins_url( 'pricing-cards-best-vpn.admin.js', __FILE__ ),
            array( 'wp-blocks', 'wp-editor', 'wp-element', 'wp-data', 'wp-components' )
        );

        register_block_type('safervpn/pricing-cards-best-vpn', array(
...
add_action('init', array('SafervpnPricingCardsBestVpn', 'init'), 11);